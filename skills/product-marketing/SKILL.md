...
description: "When the user wants to create or update their product marketing context document. Also use when the user mentions 'product context,' 'marketing context,' 'set up context,' 'positioning,' 'who is my target audience,' 'describe my product,' 'ICP,' 'ideal customer profile,' or wants to avoid repeating foundational information across marketing tasks. Use this at the start of any new project before using other marketing skills — it creates `.agents/product-marketing.md` that all other skills reference for product, audience, and positioning context. Also trigger on: 'descrever meu produto,' 'definir meu público,' 'quem é meu cliente ideal,' 'montar o posicionamento da marca,' 'organizar as informações do meu negócio,' 'não quero repetir tudo toda vez.'"
version: 1.0
---
...
**MestreOS:** if you are inside a MestreOS OS (the root claude.md lists the user's contexts), the brand's marketing context lives at `{context}/marketing.md` (or `marketing-{slug}.md` for extra brands), with a `## Marketing` pointer in that context's `index.md`. Ask which context the brand belongs to if unclear, and never mix brands — and read every reference to `.agents/product-marketing.md` in this skill as that per-context path. Outside a MestreOS OS, use `.agents/product-marketing.md` as below.
