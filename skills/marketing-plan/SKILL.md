...
name: marketing-plan
description: >-
  When the user needs a comprehensive marketing plan for a client, a company
  they advise, or their own product. Also use when the user mentions "marketing
  plan," "growth plan," "GTM plan," "go-to-market plan," "AARRR plan," "90-day
  marketing plan," "12-month marketing roadmap," "fractional CMO plan," or
  "fCMO plan." Generates an exhaustive 13-section plan structured by AARRR
  (Acquisition, Activation, Retention, Referral, Revenue), customized to the
  client's current budget, team, and stage, mapped to future funding
  ...
  see onboarding, signup, emails, referrals, pricing. Also trigger on: "montar
  um plano de marketing", "planejar o marketing do ano", "plano de 90 dias pro
  meu negocio", "organizar meu marketing do zero", "estrategia completa de
  crescimento", "roteiro de marketing pra minha empresa".
version: 1.0
---
...
2. **`product-marketing`** — Sets up the foundational marketing context file (positioning, ICP, voice). MestreOS: if you are inside a MestreOS OS (the root claude.md lists the user's contexts), the brand's marketing context lives at `{context}/marketing.md` (or `marketing-{slug}.md` for extra brands), with a `## Marketing` pointer in that context's index.md — ask which context the brand belongs to if unclear, and never mix brands. Outside a MestreOS OS, the context file is `.agents/product-marketing.md`. Read it first; Section 2 (Strategic frame) builds on it.
...
- **`product-marketing`** — Run first. Captures positioning, ICP, voice in the brand's marketing context file (MestreOS: `{context}/marketing.md`, or `marketing-{slug}.md` for extra brands, with a `## Marketing` pointer in that context's index.md; outside a MestreOS OS: `.agents/product-marketing.md`) so every section of the plan references the same foundation.
